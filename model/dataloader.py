...
class Mydataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open('data/coco_data/' + img_path)
...
class CoCoDataset_aug_update(Dataset):
    def __init__(self, data_path, transform = None, args=None):
        self.transform = transform
    ...
    def __getitem__(self, item_idx):
        item_data = self.dataset[item_idx]
        img_path = item_data['image_path']

        # The image path in the JSON might be relative to a different root
        try:
            img = Image.open('data/coco_data/' + img_path).convert('RGB')
        ...
        true_triples = item_data.get('true_triples', [])
        false_triples = item_data.get('false_triples', [])
        ...
        true_caption = tokenize(item_data.get('true_caption', ''), truncate=True)
        false_caption = tokenize(item_data.get('false_caption', ''), truncate=True)
